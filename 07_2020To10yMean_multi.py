...
def meanYear(startYear,endYear,month,day):
    Fy, Fz, nF = makeEPflux(startYear,month,day)
...
def draw(month,day):
    ...
    for y in range(2):
        if y == 0:
            Fy, Fz, nablaF = meanYear(meanstart,meanend,month,day)
            title = '10years mean'
        else:
            Fy, Fz, nablaF = makeEPflux(year,month,day)
            title = str(year)
...
def main():
    for month in range(sdate[0],edate[0]+1):
        if month == sdate[0]:
            day = sdate[1]
